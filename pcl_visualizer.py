...
def getDecodeEnvPointCloud(input,color):
    ...
    with torch.no_grad():
        encoder = Encoder()
        # encoder.load_state_dict(torch.load("models/complex_cae_encoder.pkl"))
        # encoder.load_state_dict(torch.load("models/cae_offsetnosqrttimes2_encoder.pkl"))
        # encoder.load_state_dict(torch.load("models/replace5/replace5_augment_cae_encoder1.pkl"))
        # encoder.load_state_dict(torch.load("models/source/3d/encoder/cae_offsetmse1d5_encoder2.pkl"))
        # encoder.load_state_dict(torch.load("models/source/3d/encoder/cae_offset03_encoder1.pkl"))
        # model_path example: "models/source/3d/encoder/cae_decoder1.pkl"
        encoder.load_state_dict(torch.load("models/source/3d/encoder/cae_encoder1.pkl"))
        feature = encoder(input)
        
        decoder = Decoder()
        # decoder.load_state_dict(torch.load("models/complex_cae_decoder.pkl"))
        # decoder.load_state_dict(torch.load("models/cae_offsetnosqrttimes2_decoder.pkl"))
        # decoder.load_state_dict(torch.load("models/replace5/replace5_augment_cae_decoder1.pkl"))
        # decoder.load_state_dict(torch.load("models/source/3d/encoder/cae_offsetmse1d5_decoder2.pkl"))
        # decoder.load_state_dict(torch.load("models/source/3d/encoder/cae_offset03_decoder1.pkl"))
        decoder.load_state_dict(torch.load("models/source/3d/encoder/cae_decoder1.pkl")) 
...
def getDecodedPointCloud(input, encoder_path, decoder_path):
    with torch.no_grad():
        encoder = Encoder()
